...
lwocrypt_prov_get_keymgmt_new(const OSSL_DISPATCH *fns)
{
...
lwocrypt_prov_get_keymgmt_free(const OSSL_DISPATCH *fns)
{
    /* Pilfer the keymgmt dispatch table */
    for (; fns->function_id != 0; fns++)
...
lwocrypt_prov_get_keymgmt_import(const OSSL_DISPATCH *fns)
{
    /* Pilfer the keymgmt dispatch table */
    for (; fns->function_id != 0; fns++)
...
lwocrypt_prov_get_keymgmt_export(const OSSL_DISPATCH *fns)
{
    /* Pilfer the keymgmt dispatch table */
    for (; fns->function_id != 0; fns++)
...
{
    OSSL_FUNC_keymgmt_new_fn *kmgmt_new = lwocrypt_prov_get_keymgmt_new(fns);
    OSSL_FUNC_keymgmt_free_fn *kmgmt_free = lwocrypt_prov_get_keymgmt_free(fns);
    OSSL_FUNC_keymgmt_import_fn *kmgmt_import =
        lwocrypt_prov_get_keymgmt_import(fns);
    void *key = NULL;
