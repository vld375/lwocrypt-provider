...
#include <string.h>
#include "lwocrypt_endecoder_local.h"

#ifdef NDEBUG
#define LWOCRYPT_ENC_PRINTF(a)
#define LWOCRYPT_ENC_PRINTF2(a, b)
#define LWOCRYPT_ENC_PRINTF3(a, b, c)
#else
#define LWOCRYPT_ENC_PRINTF(a) if (getenv("LWOCRYPTENC")) printf(a)
#define LWOCRYPT_ENC_PRINTF2(a, b) if (getenv("LWOCRYPTENC")) printf(a, b)
#define LWOCRYPT_ENC_PRINTF3(a, b, c) if (getenv("LWOCRYPTENC")) printf(a, b, c)
#endif // NDEBUG

struct key2any_ctx_st {
    PROV_LWOCRYPT_CTX *provctx;
                                          ...
                                          i2d_of_void *k2d)
{
    ...
    LWOCRYPT_ENC_PRINTF("LWOCRYPT ENC provider: key_to_p8info called\n");

    if ((p8info = PKCS8_PRIV_KEY_INFO_new()) == NULL
        || (derlen = k2d(key, &der)) <= 0
        || !PKCS8_pkey_set0(p8info, OBJ_nid2obj(key_nid), 0,
			// doesn't work with lwocrypt-openssl:
                        //  params_type, params, 
			// does work/interop:
			    V_ASN1_UNDEF, NULL, 
			    der, derlen)) {
                                 ...
                                 struct key2any_ctx_st *ctx)
{
    ...
    size_t klen = 0;
    OSSL_LIB_CTX *libctx = PROV_LWOCRYPT_LIBCTX_OF(ctx->provctx);

    LWOCRYPT_ENC_PRINTF("LWOCRYPT ENC provider: p8info_to_encp8 called\n");
                              ...
                              i2d_of_void *k2d, struct key2any_ctx_st *ctx)
{
    ...
    LWOCRYPT_ENC_PRINTF("LWOCRYPT ENC provider: key_to_encp8 called\n");
...
static X509_PUBKEY *lwocryptx_key_to_pubkey(const void *key, int key_nid,
                                  void *params, int params_type,
                                  i2d_of_void k2d)
{
    /* der, derlen store the key DER output and its length */
    ...
    LWOCRYPT_ENC_PRINTF2("LWOCRYPT ENC provider: lwocryptx_key_to_pubkey called for NID %d\n", key_nid);

    if ((xpk = X509_PUBKEY_new()) == NULL
        || (derlen = k2d(key, &der)) <= 0
        || !X509_PUBKEY_set0_param(xpk, OBJ_nid2obj(key_nid),
                        V_ASN1_UNDEF, NULL, // as per logic in lwocrypt_meth.c in lwocrypt-openssl
			der, derlen)) {
        ERR_raise(ERR_LIB_USER, ERR_R_MALLOC_FAILURE);
                                    ...
                                    struct key2any_ctx_st *ctx)
{
    ...
    LWOCRYPT_ENC_PRINTF("LWOCRYPT ENC provider: key_to_epki_der_priv_bio called\n");
                                    ...
                                    struct key2any_ctx_st *ctx)
{
    ...
    LWOCRYPT_ENC_PRINTF("LWOCRYPT ENC provider: key_to_epki_pem_priv_bio called\n");
                                   ...
                                   struct key2any_ctx_st *ctx)
{
    ...
    LWOCRYPT_ENC_PRINTF("LWOCRYPT ENC provider: key_to_pki_der_priv_bio called\n");
                                   ...
                                   struct key2any_ctx_st *ctx)
{
    ...
    LWOCRYPT_ENC_PRINTF("LWOCRYPT ENC provider: key_to_pki_pem_priv_bio called\n");
...
{
    int ret = 0;
    LWOCRYPTX_KEY* okey = (LWOCRYPTX_KEY*)key;
    X509_PUBKEY *xpk = NULL;
    void *str = NULL;
    int strtype = V_ASN1_UNDEF;

    LWOCRYPT_ENC_PRINTF("LWOCRYPT ENC provider: key_to_spki_der_pub_bio called\n");

    if (p2s != NULL && !p2s(key, key_nid, ctx->save_parameters,
                            &str, &strtype))
        return 0;

    xpk = lwocryptx_key_to_pubkey(key, key_nid, str, strtype, k2d);
                                   ...
                                   struct key2any_ctx_st *ctx)
{
    ...
    LWOCRYPT_ENC_PRINTF("LWOCRYPT ENC provider: key_to_spki_pem_pub_bio called\n");
                                        ...
                                        struct key2any_ctx_st *ctx)
{
    ...
    LWOCRYPT_ENC_PRINTF("LWOCRYPT ENC provider: key_to_type_specific_der_bio called\n");
...
static int key_to_type_specific_pem_bio_cb(BIO *out, const void *key,
                                           ...
                                           struct key2any_ctx_st *ctx)
{
    LWOCRYPT_ENC_PRINTF("LWOCRYPT ENC provider: key_to_type_specific_pem_bio_cb called \n");
...
static int key_to_type_specific_pem_priv_bio(BIO *out, const void *key,
                                             ...
                                             struct key2any_ctx_st *ctx)
{
    LWOCRYPT_ENC_PRINTF("LWOCRYPT ENC provider: key_to_type_specific_pem_priv_bio called\n");
...
static int key_to_type_specific_pem_pub_bio(BIO *out, const void *key,
                                            ...
                                            struct key2any_ctx_st *ctx)
{
    LWOCRYPT_ENC_PRINTF("LWOCRYPT ENC provider: key_to_type_specific_pem_pub_bio called\n");
...
static int key_to_type_specific_pem_param_bio(BIO *out, const void *key,
                                              ...
                                              struct key2any_ctx_st *ctx)
{
    LWOCRYPT_ENC_PRINTF("LWOCRYPT ENC provider: key_to_type_specific_pem_param_bio called\n");
...
static int prepare_lwocryptx_params(const void *lwocryptxkey, int nid, int save,
                             void **pstr, int *pstrtype)
{
    ASN1_OBJECT *params = NULL;
    LWOCRYPTX_KEY *k = (LWOCRYPTX_KEY*)lwocryptxkey;

    LWOCRYPT_ENC_PRINTF3("LWOCRYPT ENC provider: prepare_lwocryptx_params called with nid %d (tlsname: %s)\n", nid, k->tls_name);
    ...
    if (nid != NID_undef) {
        ...
        ERR_raise(ERR_LIB_USER, LWOCRYPTPROV_R_MISSING_OID);
        return 0;
    }
...
static int lwocryptx_spki_pub_to_der(const void *vxkey, unsigned char **pder)
{
    const LWOCRYPTX_KEY *lwocryptxkey = vxkey;
    unsigned char *keyblob;
    int ret = 0;

    LWOCRYPT_ENC_PRINTF("LWOCRYPT ENC provider: lwocryptx_spki_pub_to_der called\n");

    if (lwocryptxkey == NULL || lwocryptxkey->pubkey == NULL) {
        ERR_raise(ERR_LIB_USER, ERR_R_PASSED_NULL_PARAMETER);
        return 0;
    }
#ifdef USE_ENCODING_LIB
    if (lwocryptxkey->lwocryptx_encoding_ctx.encoding_ctx != NULL && lwocryptxkey->lwocryptx_encoding_ctx.encoding_impl != NULL) {
        unsigned char *buf;
        int buflen;
        int ret = 0;
        const LWOCRYPTX_ENCODING_CTX* encoding_ctx = &lwocryptxkey->lwocryptx_encoding_ctx;
        buflen = encoding_ctx->encoding_impl->crypto_publickeybytes;

        buf = OPENSSL_secure_zalloc(buflen);
        ret = qsc_encode(encoding_ctx->encoding_ctx, encoding_ctx->encoding_impl, lwocryptxkey->pubkey, &buf, 0, 0, 1);
...
#endif
        keyblob = OPENSSL_memdup(lwocryptxkey->pubkey, lwocryptxkey->pubkeylen);
        if (keyblob == NULL) {
            ERR_raise(ERR_LIB_USER, ERR_R_MALLOC_FAILURE);
            return 0;
        }
        *pder = keyblob;
        return lwocryptxkey->pubkeylen;
#ifdef USE_ENCODING_LIB
    }
#endif
}

static int lwocryptx_pki_priv_to_der(const void *vxkey, unsigned char **pder)
{
    LWOCRYPTX_KEY *lwocryptxkey = (LWOCRYPTX_KEY *)vxkey;
    unsigned char* buf = NULL;
    int buflen = 0, privkeylen;
    ASN1_OCTET_STRING oct;
    int keybloblen;

    LWOCRYPT_ENC_PRINTF("LWOCRYPT ENC provider: lwocryptx_pki_priv_to_der called\n");

    // Encoding private _and_ public key concatenated ... seems unlogical and unnecessary, 
    // but is what lwocrypt-openssl does, so we repeat it for interop... also from a security 
    // perspective not really smart to copy key material (side channel attacks, anyone?),
    // but so be it for now (TBC).
    if (lwocryptxkey == NULL || lwocryptxkey->privkey == NULL 
#ifndef NOPUBKEY_IN_PRIVKEY
		    || lwocryptxkey->pubkey == NULL
#endif
        ) {
        ERR_raise(ERR_LIB_USER, ERR_R_PASSED_NULL_PARAMETER);
        return 0;
    ...
    // only concatenate private classic key (if any) and LWOCRYPT private and public key
    // NOT saving public classic key component (if any)
    privkeylen = lwocryptxkey->privkeylen;
    if (lwocryptxkey->numkeys > 1) { // hybrid
        int actualprivkeylen;
        DECODE_UINT32(actualprivkeylen, lwocryptxkey->privkey);
	if (actualprivkeylen > lwocryptxkey->evp_info->length_private_key) {
            ERR_raise(ERR_LIB_USER, LWOCRYPTPROV_R_INVALID_ENCODING);
            return 0;
	}
	privkeylen -= (lwocryptxkey->evp_info->length_private_key - actualprivkeylen);
    }
#ifdef USE_ENCODING_LIB
    if (lwocryptxkey->lwocryptx_encoding_ctx.encoding_ctx != NULL && lwocryptxkey->lwocryptx_encoding_ctx.encoding_impl != NULL) {
        const LWOCRYPTX_ENCODING_CTX* encoding_ctx = &lwocryptxkey->lwocryptx_encoding_ctx;
        ...
        ret = qsc_encode(encoding_ctx->encoding_ctx, encoding_ctx->encoding_impl, 
            lwocryptxkey->comp_pubkey[lwocryptxkey->numkeys-1], 0, 
            lwocryptxkey->privkey, &buf, withoptional);
        if (ret != QSC_ENC_OK) return -1;
    } else {
#endif
#ifdef NOPUBKEY_IN_PRIVKEY
        ...
        LWOCRYPT_ENC_PRINTF2("LWOCRYPT ENC provider: saving privkey of length %d\n", buflen);
        memcpy(buf, lwocryptxkey->privkey, privkeylen);
#else
        buflen = privkeylen+lwocryptx_key_get_lwocrypt_public_key_len(lwocryptxkey);
        buf = OPENSSL_secure_malloc(buflen);
        LWOCRYPT_ENC_PRINTF2("LWOCRYPT ENC provider: saving priv+pubkey of length %d\n", buflen);
        memcpy(buf, lwocryptxkey->privkey, privkeylen);
        memcpy(buf+privkeylen, lwocryptxkey->comp_pubkey[lwocryptxkey->numkeys-1], lwocryptx_key_get_lwocrypt_public_key_len(lwocryptxkey));
#endif
#ifdef USE_ENCODING_LIB
    }
#endif
    ...
    oct.length = buflen;
    // more logical:
    //oct.data = lwocryptxkey->privkey;
    //oct.length = lwocryptxkey->privkeylen;
    oct.flags = 0;
...
# define lwocryptx_epki_priv_to_der lwocryptx_pki_priv_to_der

/*
 * LWOCRYPTX only has PKCS#8 / SubjectPublicKeyInfo
 * representation, so we don't define lwocryptx_type_specific_[priv,pub,params]_to_der.
 */

# define lwocryptx_check_key_type     NULL

// LWOCRYPT provider uses NIDs generated at load time as EVP_type identifiers
// so initially this must be 0 and set to a real value by OBJ_sn2nid later
///// LWOCRYPT_TEMPLATE_FRAGMENT_ENCODER_DEFINES_START
...
///// LWOCRYPT_TEMPLATE_FRAGMENT_ENCODER_DEFINES_END
...
static void *key2any_newctx(void *provctx)
{
    ...
    LWOCRYPT_ENC_PRINTF("LWOCRYPT ENC provider: key2any_newctx called\n");
...
static void key2any_freectx(void *vctx)
{
    ...
    LWOCRYPT_ENC_PRINTF("LWOCRYPT ENC provider: key2any_freectx called\n");
...
static const OSSL_PARAM *key2any_settable_ctx_params(ossl_unused void *provctx)
{
    ...
    LWOCRYPT_ENC_PRINTF("LWOCRYPT ENC provider: key2any_settable_ctx_params called\n");
...
static int key2any_set_ctx_params(void *vctx, const OSSL_PARAM params[])
{
    ...
    LWOCRYPT_ENC_PRINTF("LWOCRYPT ENC provider: key2any_set_ctx_params called\n");
    ...
    if (cipherp != NULL) {
        ...
        if (!OSSL_PARAM_get_utf8_string_ptr(cipherp, &ciphername))
            return 0;
        LWOCRYPT_ENC_PRINTF2(" setting cipher: %s\n", ciphername);
    ...
    LWOCRYPT_ENC_PRINTF2(" cipher set to %p: \n", ctx->cipher);
    return 1;
}
...
static int key2any_check_selection(int selection, int selection_mask)
{
    ...
    LWOCRYPT_ENC_PRINTF3("LWOCRYPT ENC provider: key2any_check_selection called with selection %d (%d)\n",selection, selection_mask);
    ...
    for (i = 0; i < OSSL_NELEM(checks); i++) {
         ...
         */
        if (check1) {
    LWOCRYPT_ENC_PRINTF2("LWOCRYPT ENC provider: key2any_check_selection returns %d\n", check2);
            return check2;
	}
    }
                          ...
                          i2d_of_void *key2der)
{
    ...
    LWOCRYPTX_KEY *lwocryptk = (LWOCRYPTX_KEY*)key;

    LWOCRYPT_ENC_PRINTF3("LWOCRYPT ENC provider: key2any_encode called with type %d (%s)\n", type, typestr);
    LWOCRYPT_ENC_PRINTF2("LWOCRYPT ENC provider: key2any_encode called with pemname %s\n", pemname);

    if (key == NULL || type <= 0) {
        ERR_raise(ERR_LIB_USER, ERR_R_PASSED_NULL_PARAMETER);
    } else if (writer != NULL) {
        // Is ref counting really needed? For now, do it as per https://beta.openssl.org/docs/manmaster/man3/BIO_new_from_core_bio.html:
        BIO *out = lwocrypt_bio_new_from_core_bio(ctx->provctx, cout);
    ...
    LWOCRYPT_ENC_PRINTF2(" encode result: %d\n", ret);
    return ret;
}
 ...
 *
 * lwocrypt_##impl##_to_##kind##_##output##_encoder_functions
 */
#define MAKE_ENCODER(impl, type, kind, output)                    \
    ...
    {                                                                       \
        struct key2any_ctx_st *ctx = vctx;                                  \
                                                                            \
        LWOCRYPT_ENC_PRINTF("LWOCRYPT ENC provider: _import_object called\n"); \
        return lwocrypt_prov_import_key(lwocrypt_##impl##_keymgmt_functions,        \
                                    ctx->provctx, selection, params);       \
    }                                                                       \
    static void impl##_to_##kind##_##output##_free_object(void *key)        \
    {                                                                       \
        LWOCRYPT_ENC_PRINTF("LWOCRYPT ENC provider: _free_object called\n"); \
        lwocrypt_prov_free_key(lwocrypt_##impl##_keymgmt_functions, key);           \
    }                                                                       \
    static int impl##_to_##kind##_##output##_does_selection(void *ctx,      \
                                                            int selection)  \
    {                                                                       \
        LWOCRYPT_ENC_PRINTF("LWOCRYPT ENC provider: _does_selection called\n"); \
    ...
    {                                                                       \
        ...
        LWOCRYPT_ENC_PRINTF("LWOCRYPT ENC provider: _encode called\n"); \
...
static int lwocryptx_to_text(BIO *out, const void *key, int selection)
{
    LWOCRYPTX_KEY* okey = (LWOCRYPTX_KEY*)key;
    int is_hybrid = 0;
...
{
    BIO *out = lwocrypt_bio_new_from_core_bio(vctx, cout);
    int ret;
...
#define MAKE_TEXT_ENCODER(impl)                                         \
    ...
    {                                                                   \
        ...
        return key2text_encode(vctx, key, selection, cout,              \
                               lwocryptx_to_text, cb, cbarg);                \
    }                                                                   \
    const OSSL_DISPATCH lwocrypt_##impl##_to_text_encoder_functions[] = {    \
        { OSSL_FUNC_ENCODER_NEWCTX,                                     \
 ...
 */
///// LWOCRYPT_TEMPLATE_FRAGMENT_ENCODER_MAKE_START
MAKE_ENCODER(dilithium2, lwocryptx, EncryptedPrivateKeyInfo, der);
MAKE_ENCODER(dilithium2, lwocryptx, EncryptedPrivateKeyInfo, pem);
MAKE_ENCODER(dilithium2, lwocryptx, PrivateKeyInfo, der);
MAKE_ENCODER(dilithium2, lwocryptx, PrivateKeyInfo, pem);
MAKE_ENCODER(dilithium2, lwocryptx, SubjectPublicKeyInfo, der);
MAKE_ENCODER(dilithium2, lwocryptx, SubjectPublicKeyInfo, pem);
MAKE_TEXT_ENCODER(dilithium2);
MAKE_ENCODER(p256_dilithium2, lwocryptx, EncryptedPrivateKeyInfo, der);
MAKE_ENCODER(p256_dilithium2, lwocryptx, EncryptedPrivateKeyInfo, pem);
MAKE_ENCODER(p256_dilithium2, lwocryptx, PrivateKeyInfo, der);
MAKE_ENCODER(p256_dilithium2, lwocryptx, PrivateKeyInfo, pem);
MAKE_ENCODER(p256_dilithium2, lwocryptx, SubjectPublicKeyInfo, der);
MAKE_ENCODER(p256_dilithium2, lwocryptx, SubjectPublicKeyInfo, pem);
MAKE_TEXT_ENCODER(p256_dilithium2);
MAKE_ENCODER(rsa3072_dilithium2, lwocryptx, EncryptedPrivateKeyInfo, der);
MAKE_ENCODER(rsa3072_dilithium2, lwocryptx, EncryptedPrivateKeyInfo, pem);
MAKE_ENCODER(rsa3072_dilithium2, lwocryptx, PrivateKeyInfo, der);
MAKE_ENCODER(rsa3072_dilithium2, lwocryptx, PrivateKeyInfo, pem);
MAKE_ENCODER(rsa3072_dilithium2, lwocryptx, SubjectPublicKeyInfo, der);
MAKE_ENCODER(rsa3072_dilithium2, lwocryptx, SubjectPublicKeyInfo, pem);
MAKE_TEXT_ENCODER(rsa3072_dilithium2);
MAKE_ENCODER(dilithium3, lwocryptx, EncryptedPrivateKeyInfo, der);
MAKE_ENCODER(dilithium3, lwocryptx, EncryptedPrivateKeyInfo, pem);
MAKE_ENCODER(dilithium3, lwocryptx, PrivateKeyInfo, der);
MAKE_ENCODER(dilithium3, lwocryptx, PrivateKeyInfo, pem);
MAKE_ENCODER(dilithium3, lwocryptx, SubjectPublicKeyInfo, der);
MAKE_ENCODER(dilithium3, lwocryptx, SubjectPublicKeyInfo, pem);
MAKE_TEXT_ENCODER(dilithium3);
MAKE_ENCODER(p384_dilithium3, lwocryptx, EncryptedPrivateKeyInfo, der);
MAKE_ENCODER(p384_dilithium3, lwocryptx, EncryptedPrivateKeyInfo, pem);
MAKE_ENCODER(p384_dilithium3, lwocryptx, PrivateKeyInfo, der);
MAKE_ENCODER(p384_dilithium3, lwocryptx, PrivateKeyInfo, pem);
MAKE_ENCODER(p384_dilithium3, lwocryptx, SubjectPublicKeyInfo, der);
MAKE_ENCODER(p384_dilithium3, lwocryptx, SubjectPublicKeyInfo, pem);
MAKE_TEXT_ENCODER(p384_dilithium3);
MAKE_ENCODER(dilithium5, lwocryptx, EncryptedPrivateKeyInfo, der);
MAKE_ENCODER(dilithium5, lwocryptx, EncryptedPrivateKeyInfo, pem);
MAKE_ENCODER(dilithium5, lwocryptx, PrivateKeyInfo, der);
MAKE_ENCODER(dilithium5, lwocryptx, PrivateKeyInfo, pem);
MAKE_ENCODER(dilithium5, lwocryptx, SubjectPublicKeyInfo, der);
MAKE_ENCODER(dilithium5, lwocryptx, SubjectPublicKeyInfo, pem);
MAKE_TEXT_ENCODER(dilithium5);
MAKE_ENCODER(p521_dilithium5, lwocryptx, EncryptedPrivateKeyInfo, der);
MAKE_ENCODER(p521_dilithium5, lwocryptx, EncryptedPrivateKeyInfo, pem);
MAKE_ENCODER(p521_dilithium5, lwocryptx, PrivateKeyInfo, der);
MAKE_ENCODER(p521_dilithium5, lwocryptx, PrivateKeyInfo, pem);
MAKE_ENCODER(p521_dilithium5, lwocryptx, SubjectPublicKeyInfo, der);
MAKE_ENCODER(p521_dilithium5, lwocryptx, SubjectPublicKeyInfo, pem);
MAKE_TEXT_ENCODER(p521_dilithium5);
MAKE_ENCODER(falcon512, lwocryptx, EncryptedPrivateKeyInfo, der);
MAKE_ENCODER(falcon512, lwocryptx, EncryptedPrivateKeyInfo, pem);
MAKE_ENCODER(falcon512, lwocryptx, PrivateKeyInfo, der);
MAKE_ENCODER(falcon512, lwocryptx, PrivateKeyInfo, pem);
MAKE_ENCODER(falcon512, lwocryptx, SubjectPublicKeyInfo, der);
MAKE_ENCODER(falcon512, lwocryptx, SubjectPublicKeyInfo, pem);
MAKE_TEXT_ENCODER(falcon512);
MAKE_ENCODER(p256_falcon512, lwocryptx, EncryptedPrivateKeyInfo, der);
MAKE_ENCODER(p256_falcon512, lwocryptx, EncryptedPrivateKeyInfo, pem);
MAKE_ENCODER(p256_falcon512, lwocryptx, PrivateKeyInfo, der);
MAKE_ENCODER(p256_falcon512, lwocryptx, PrivateKeyInfo, pem);
MAKE_ENCODER(p256_falcon512, lwocryptx, SubjectPublicKeyInfo, der);
MAKE_ENCODER(p256_falcon512, lwocryptx, SubjectPublicKeyInfo, pem);
MAKE_TEXT_ENCODER(p256_falcon512);
MAKE_ENCODER(rsa3072_falcon512, lwocryptx, EncryptedPrivateKeyInfo, der);
MAKE_ENCODER(rsa3072_falcon512, lwocryptx, EncryptedPrivateKeyInfo, pem);
MAKE_ENCODER(rsa3072_falcon512, lwocryptx, PrivateKeyInfo, der);
MAKE_ENCODER(rsa3072_falcon512, lwocryptx, PrivateKeyInfo, pem);
MAKE_ENCODER(rsa3072_falcon512, lwocryptx, SubjectPublicKeyInfo, der);
MAKE_ENCODER(rsa3072_falcon512, lwocryptx, SubjectPublicKeyInfo, pem);
MAKE_TEXT_ENCODER(rsa3072_falcon512);
MAKE_ENCODER(falcon1024, lwocryptx, EncryptedPrivateKeyInfo, der);
MAKE_ENCODER(falcon1024, lwocryptx, EncryptedPrivateKeyInfo, pem);
MAKE_ENCODER(falcon1024, lwocryptx, PrivateKeyInfo, der);
MAKE_ENCODER(falcon1024, lwocryptx, PrivateKeyInfo, pem);
MAKE_ENCODER(falcon1024, lwocryptx, SubjectPublicKeyInfo, der);
MAKE_ENCODER(falcon1024, lwocryptx, SubjectPublicKeyInfo, pem);
MAKE_TEXT_ENCODER(falcon1024);
MAKE_ENCODER(p521_falcon1024, lwocryptx, EncryptedPrivateKeyInfo, der);
MAKE_ENCODER(p521_falcon1024, lwocryptx, EncryptedPrivateKeyInfo, pem);
MAKE_ENCODER(p521_falcon1024, lwocryptx, PrivateKeyInfo, der);
MAKE_ENCODER(p521_falcon1024, lwocryptx, PrivateKeyInfo, pem);
MAKE_ENCODER(p521_falcon1024, lwocryptx, SubjectPublicKeyInfo, der);
MAKE_ENCODER(p521_falcon1024, lwocryptx, SubjectPublicKeyInfo, pem);
MAKE_TEXT_ENCODER(p521_falcon1024);
MAKE_ENCODER(sphincssha2128fsimple, lwocryptx, EncryptedPrivateKeyInfo, der);
MAKE_ENCODER(sphincssha2128fsimple, lwocryptx, EncryptedPrivateKeyInfo, pem);
MAKE_ENCODER(sphincssha2128fsimple, lwocryptx, PrivateKeyInfo, der);
MAKE_ENCODER(sphincssha2128fsimple, lwocryptx, PrivateKeyInfo, pem);
MAKE_ENCODER(sphincssha2128fsimple, lwocryptx, SubjectPublicKeyInfo, der);
MAKE_ENCODER(sphincssha2128fsimple, lwocryptx, SubjectPublicKeyInfo, pem);
MAKE_TEXT_ENCODER(sphincssha2128fsimple);
MAKE_ENCODER(p256_sphincssha2128fsimple, lwocryptx, EncryptedPrivateKeyInfo, der);
MAKE_ENCODER(p256_sphincssha2128fsimple, lwocryptx, EncryptedPrivateKeyInfo, pem);
MAKE_ENCODER(p256_sphincssha2128fsimple, lwocryptx, PrivateKeyInfo, der);
MAKE_ENCODER(p256_sphincssha2128fsimple, lwocryptx, PrivateKeyInfo, pem);
MAKE_ENCODER(p256_sphincssha2128fsimple, lwocryptx, SubjectPublicKeyInfo, der);
MAKE_ENCODER(p256_sphincssha2128fsimple, lwocryptx, SubjectPublicKeyInfo, pem);
MAKE_TEXT_ENCODER(p256_sphincssha2128fsimple);
MAKE_ENCODER(rsa3072_sphincssha2128fsimple, lwocryptx, EncryptedPrivateKeyInfo, der);
MAKE_ENCODER(rsa3072_sphincssha2128fsimple, lwocryptx, EncryptedPrivateKeyInfo, pem);
MAKE_ENCODER(rsa3072_sphincssha2128fsimple, lwocryptx, PrivateKeyInfo, der);
MAKE_ENCODER(rsa3072_sphincssha2128fsimple, lwocryptx, PrivateKeyInfo, pem);
MAKE_ENCODER(rsa3072_sphincssha2128fsimple, lwocryptx, SubjectPublicKeyInfo, der);
MAKE_ENCODER(rsa3072_sphincssha2128fsimple, lwocryptx, SubjectPublicKeyInfo, pem);
MAKE_TEXT_ENCODER(rsa3072_sphincssha2128fsimple);
MAKE_ENCODER(sphincssha2128ssimple, lwocryptx, EncryptedPrivateKeyInfo, der);
MAKE_ENCODER(sphincssha2128ssimple, lwocryptx, EncryptedPrivateKeyInfo, pem);
MAKE_ENCODER(sphincssha2128ssimple, lwocryptx, PrivateKeyInfo, der);
MAKE_ENCODER(sphincssha2128ssimple, lwocryptx, PrivateKeyInfo, pem);
MAKE_ENCODER(sphincssha2128ssimple, lwocryptx, SubjectPublicKeyInfo, der);
MAKE_ENCODER(sphincssha2128ssimple, lwocryptx, SubjectPublicKeyInfo, pem);
MAKE_TEXT_ENCODER(sphincssha2128ssimple);
MAKE_ENCODER(p256_sphincssha2128ssimple, lwocryptx, EncryptedPrivateKeyInfo, der);
MAKE_ENCODER(p256_sphincssha2128ssimple, lwocryptx, EncryptedPrivateKeyInfo, pem);
MAKE_ENCODER(p256_sphincssha2128ssimple, lwocryptx, PrivateKeyInfo, der);
MAKE_ENCODER(p256_sphincssha2128ssimple, lwocryptx, PrivateKeyInfo, pem);
MAKE_ENCODER(p256_sphincssha2128ssimple, lwocryptx, SubjectPublicKeyInfo, der);
MAKE_ENCODER(p256_sphincssha2128ssimple, lwocryptx, SubjectPublicKeyInfo, pem);
MAKE_TEXT_ENCODER(p256_sphincssha2128ssimple);
MAKE_ENCODER(rsa3072_sphincssha2128ssimple, lwocryptx, EncryptedPrivateKeyInfo, der);
MAKE_ENCODER(rsa3072_sphincssha2128ssimple, lwocryptx, EncryptedPrivateKeyInfo, pem);
MAKE_ENCODER(rsa3072_sphincssha2128ssimple, lwocryptx, PrivateKeyInfo, der);
MAKE_ENCODER(rsa3072_sphincssha2128ssimple, lwocryptx, PrivateKeyInfo, pem);
MAKE_ENCODER(rsa3072_sphincssha2128ssimple, lwocryptx, SubjectPublicKeyInfo, der);
MAKE_ENCODER(rsa3072_sphincssha2128ssimple, lwocryptx, SubjectPublicKeyInfo, pem);
MAKE_TEXT_ENCODER(rsa3072_sphincssha2128ssimple);
MAKE_ENCODER(sphincssha2192fsimple, lwocryptx, EncryptedPrivateKeyInfo, der);
MAKE_ENCODER(sphincssha2192fsimple, lwocryptx, EncryptedPrivateKeyInfo, pem);
MAKE_ENCODER(sphincssha2192fsimple, lwocryptx, PrivateKeyInfo, der);
MAKE_ENCODER(sphincssha2192fsimple, lwocryptx, PrivateKeyInfo, pem);
MAKE_ENCODER(sphincssha2192fsimple, lwocryptx, SubjectPublicKeyInfo, der);
MAKE_ENCODER(sphincssha2192fsimple, lwocryptx, SubjectPublicKeyInfo, pem);
MAKE_TEXT_ENCODER(sphincssha2192fsimple);
MAKE_ENCODER(p384_sphincssha2192fsimple, lwocryptx, EncryptedPrivateKeyInfo, der);
MAKE_ENCODER(p384_sphincssha2192fsimple, lwocryptx, EncryptedPrivateKeyInfo, pem);
MAKE_ENCODER(p384_sphincssha2192fsimple, lwocryptx, PrivateKeyInfo, der);
MAKE_ENCODER(p384_sphincssha2192fsimple, lwocryptx, PrivateKeyInfo, pem);
MAKE_ENCODER(p384_sphincssha2192fsimple, lwocryptx, SubjectPublicKeyInfo, der);
MAKE_ENCODER(p384_sphincssha2192fsimple, lwocryptx, SubjectPublicKeyInfo, pem);
MAKE_TEXT_ENCODER(p384_sphincssha2192fsimple);
MAKE_ENCODER(sphincsshake128fsimple, lwocryptx, EncryptedPrivateKeyInfo, der);
MAKE_ENCODER(sphincsshake128fsimple, lwocryptx, EncryptedPrivateKeyInfo, pem);
MAKE_ENCODER(sphincsshake128fsimple, lwocryptx, PrivateKeyInfo, der);
MAKE_ENCODER(sphincsshake128fsimple, lwocryptx, PrivateKeyInfo, pem);
MAKE_ENCODER(sphincsshake128fsimple, lwocryptx, SubjectPublicKeyInfo, der);
MAKE_ENCODER(sphincsshake128fsimple, lwocryptx, SubjectPublicKeyInfo, pem);
MAKE_TEXT_ENCODER(sphincsshake128fsimple);
MAKE_ENCODER(p256_sphincsshake128fsimple, lwocryptx, EncryptedPrivateKeyInfo, der);
MAKE_ENCODER(p256_sphincsshake128fsimple, lwocryptx, EncryptedPrivateKeyInfo, pem);
MAKE_ENCODER(p256_sphincsshake128fsimple, lwocryptx, PrivateKeyInfo, der);
MAKE_ENCODER(p256_sphincsshake128fsimple, lwocryptx, PrivateKeyInfo, pem);
MAKE_ENCODER(p256_sphincsshake128fsimple, lwocryptx, SubjectPublicKeyInfo, der);
MAKE_ENCODER(p256_sphincsshake128fsimple, lwocryptx, SubjectPublicKeyInfo, pem);
MAKE_TEXT_ENCODER(p256_sphincsshake128fsimple);
MAKE_ENCODER(rsa3072_sphincsshake128fsimple, lwocryptx, EncryptedPrivateKeyInfo, der);
MAKE_ENCODER(rsa3072_sphincsshake128fsimple, lwocryptx, EncryptedPrivateKeyInfo, pem);
MAKE_ENCODER(rsa3072_sphincsshake128fsimple, lwocryptx, PrivateKeyInfo, der);
MAKE_ENCODER(rsa3072_sphincsshake128fsimple, lwocryptx, PrivateKeyInfo, pem);
MAKE_ENCODER(rsa3072_sphincsshake128fsimple, lwocryptx, SubjectPublicKeyInfo, der);
MAKE_ENCODER(rsa3072_sphincsshake128fsimple, lwocryptx, SubjectPublicKeyInfo, pem);
MAKE_TEXT_ENCODER(rsa3072_sphincsshake128fsimple);
///// LWOCRYPT_TEMPLATE_FRAGMENT_ENCODER_MAKE_END
