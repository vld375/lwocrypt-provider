...
def get_kem_nistlevel(alg):
    if 'LIBLWOCRYPT_SRC_DIR' not in os.environ:
        print("Must include LIBLWOCRYPT_SRC_DIR in environment")
        exit(1)
    ...
    # load datasheet
    algymlfilename = os.path.join(os.environ['LIBLWOCRYPT_SRC_DIR'], 'docs', 'algorithms', 'kem', '{:s}.yml'.format(datasheetname))
...
def get_sig_nistlevel(family, alg):
    ...
    # load datasheet
    algymlfilename = os.path.join(os.environ['LIBLWOCRYPT_SRC_DIR'], 'docs', 'algorithms', 'sig', '{:s}.yml'.format(datasheetname))
    algyml = yaml.safe_load(file_get_contents(algymlfilename, encoding='utf-8'))
    # hacks to match names
    def matches(name, alg):
        def simplify(s):
...
def populate(filename, config, delimiter, overwrite=False):
    fragments = glob.glob(os.path.join('lwocrypt-template', filename, '*.fragment'))
    if overwrite == True:
        source_file = os.path.join('lwocrypt-template', filename, os.path.basename(filename)+ '.base')
    ...
    for fragment in fragments:
        ...
        if filename.endswith('.md'):
            identifier_start = '{} LWOCRYPT_TEMPLATE_FRAGMENT_{}_START -->'.format(delimiter, identifier.upper())
        else:
            identifier_start = '{} LWOCRYPT_TEMPLATE_FRAGMENT_{}_START'.format(delimiter, identifier.upper())
        identifier_end = '{} LWOCRYPT_TEMPLATE_FRAGMENT_{}_END'.format(delimiter, identifier.upper())
...
def load_config(include_disabled_sigs=False):
    config = file_get_contents(os.path.join('lwocrypt-template', 'generate.yml'), encoding='utf-8')
...
populate('lwocryptprov/lwocryptencoders.inc', config, '/////')
populate('lwocryptprov/lwocryptdecoders.inc', config, '/////')
populate('lwocryptprov/lwocrypt_prov.h', config, '/////')
populate('lwocryptprov/lwocryptprov.c', config, '/////')
populate('lwocryptprov/lwocryptprov_capabilities.c', config, '/////')
populate('lwocryptprov/lwocrypt_kmgmt.c', config, '/////')
populate('lwocryptprov/lwocrypt_encode_key2any.c', config, '/////')
populate('lwocryptprov/lwocrypt_decode_der2key.c', config, '/////')
populate('lwocryptprov/lwocryptprov_keys.c', config, '/////')
...
os.environ["LIBLWOCRYPT_DOCS_DIR"]=os.path.join(os.environ["LIBLWOCRYPT_SRC_DIR"], "docs")
